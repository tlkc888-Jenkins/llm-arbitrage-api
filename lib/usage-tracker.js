...
async function getPersistedStats() {
  ...
  try {
    // Get total counts by event type (include user_agent for analysis)
    const eventsRes = await fetch(
      `${SUPABASE_URL}/rest/v1/usage_events?select=event_type,server_slug,tool_name,created_at,user_agent,ip_hash&order=created_at.desc&limit=500`,
      {
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`,
